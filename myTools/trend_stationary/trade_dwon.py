# -*- coding: utf-8 -*-
"""
Created on Wed Apr 21 13:22:05 2021
...
# 標準差倍數當作停損門檻(滑價＋交易稅)-------------------------------------------------------------------------------
path_to_image = "./total_profit_plot/0104/"
def pairs(pair,slope, formate_time, table, min_data, tick_data, up_open_time, down_open_time, stop_loss_time, day1, maxi,
    ...
    stock1_seq = min_price[str(int(table.stock1[pair]))].loc[0:t]
    stock2_seq = min_price[str(int(table.stock2[pair]))].loc[0:t]
    ...
    local_skew = []
    local_timetrend = []
    spread = table.w1[pair] * np.log(tick_data[str(int(table.stock1[pair]))]) + table.w2[pair] * np.log(
        tick_data[str(int(table.stock2[pair]))])
    ...
    for i in range(0, len(spread) - 2):
        #print(spread[i],close,up_open)
        stock1_seq = min_price[str(int(table.stock1[pair]))].loc[0:t + i]
        stock2_seq = min_price[str(int(table.stock2[pair]))].loc[0:t + i]
        # z = ( np.vstack( [stock1_seq , stock2_seq] ).T )
        ...
        if position == 0 and i != len(spread) - 3 and i < 40:  # 之前無開倉
            ...
            if  spread[i] < (close_list[i] + stop_loss) and  spread[i] > up_open_list[i] :  # 碰到上開倉門檻且小於上停損門檻
                # 資金權重轉股票張數，並整數化
                w1, w2 = num_weight(table.w1[pair], table.w2[pair], tick_data[str(int(table.stock1[pair]))][i],
                                    tick_data[str(int(table.stock2[pair]))][i], maxi, capital)
                ...
                if use_adf and adfuller(spread1, regression='c')[1] > 0.05:  # spread平穩才開倉
                    ...
                    position = -1
                    stock1_payoff = w1 * slip(tick_data[str(int(table.stock1[pair]))][(i)], table.w1[pair])
                    stock2_payoff = w2 * slip(tick_data[str(int(table.stock2[pair]))][(i)], table.w2[pair])
            ...
            if i == (len(spread) - 3):  # 回測結束，強制平倉
                ...
                stock1_payoff, stock2_payoff = tax(stock1_payoff, stock2_payoff, position, tax_cost)  # 計算交易成本
                trading[2] += 1
            elif (spread[i] - close_list[i]) < 0:  # 空倉碰到下開倉門檻即平倉
                position = 666  # 平倉
                stock1_payoff = -w1 * slip(tick_data[str(int(table.stock1[pair]))][i], -table.w1[pair])
                stock2_payoff = -w2 * slip(tick_data[str(int(table.stock2[pair]))][i], -table.w2[pair])
                stock1_payoff, stock2_payoff = tax(stock1_payoff, stock2_payoff, position, tax_cost)  # 計算交易成本
            ...
            elif spread[i] > (close_list[i] + stop_loss):  # 空倉碰到上停損門檻即平倉停損
                ...
                stock1_payoff = -w1 * slip(tick_data[str(int(table.stock1[pair]))][(i + 1)], -table.w1[pair])
                stock2_payoff = -w2 * slip(tick_data[str(int(table.stock2[pair]))][(i + 1)], -table.w2[pair])
                stock1_payoff, stock2_payoff = tax(stock1_payoff, stock2_payoff, position, tax_cost)  # 計算交易成本
                trading[2]+=1
            ...
            if i == (len(spread) - 3):  # 回測結束，強制平倉
                ...
                # 每次交易報酬做累加(最後除以交易次數做平均)
            elif spread[i] < (close_list[i] - stop_loss):
                position = -2  # 碰到停損門檻，強制平倉
                stock1_payoff = w1 * slip(tick_data[str(int(table.stock1[pair]))][(i)], table.w1[pair])
                stock2_payoff = w2 * slip(tick_data[str(int(table.stock2[pair]))][(i)], table.w2[pair])
                stock1_payoff, stock2_payoff = tax(stock1_payoff, stock2_payoff, position, tax_cost)  # 計算交易成本
    ...
    if trade == 0:  # 如果都沒有開倉，則報酬為0
        ...
        spread2 = w1 * np.log(min_price[str(int(table.stock1[pair]))].iloc[0:t]) + w2 * np.log(
            min_price[str(int(table.stock2[pair]))].iloc[0:t])
    ...
    if True :# (position == -2 or position == -3 or position == -4) and local_profit <= -10 :
        if dump:
            plot_spread( table.stock1[pair], table.stock2[pair], spread, up_open_list, down_open_list, stop_loss,
                        close_list, local_profit, pos, position, up_open_time, down_open_time, stop_loss_time)
    return   local_profit, local_open_num, trade_capital, trading
